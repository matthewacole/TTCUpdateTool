...
import { filterAlertsByPriority } from "../api/alerts";
...
export function DashboardWidget({ onAddStop }: DashboardWidgetProps) {
  ...
  const [filterMode, setFilterMode] = useState<"all" | "priority">(preferences.get().alertFilterMode);
  const [priorityIds, setPriorityIds] = useState<Set<number>>(new Set(preferences.getPriorityRouteIds()));
  ...
  const fetchAlerts = useCallback(async () => {
    try {
      const prefs = preferences.get();
      setFilterMode(prefs.alertFilterMode);
      setPriorityIds(new Set(preferences.getPriorityRouteIds()));
  ...
  const filteredAlerts = filterMode === "priority"
    ? filterAlertsByPriority(alerts, priorityIds)
    : alerts;
  ...
  const displayAlerts = filterMode === "priority" ? filteredAlerts : alerts;
        ...
        <WidgetBase size="large">
          <div class="at">
            <div class="at__header">
              ...
              <button class="dw__settings-btn" onClick={() => setShowSettings(true)} aria-label="Alert settings">⚙️</button>
              <span class="at__count">{filterMode === "priority" ? displayAlerts.length : alerts.length}</span>
            </div>
            {filterMode === "priority" && displayAlerts.length === 0 ? (
              <div class="at__empty-filtered">
                <span class="at__hint">No priority alerts</span>
                <span class="at__hint-sub">Switch to All or manage routes in settings</span>
              </div>
            ) : (
              <div class="at__list">
                {(filterMode === "priority" ? displayAlerts : alerts).map((a) => (
