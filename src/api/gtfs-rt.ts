...
function mapAlert(entity: GtfsRtAlertEntity): ServiceAlert {
  ...
  return {
    ...
    routes: [...new Set((a.informedEntity ?? []).map((e) => e.routeId).filter(Boolean) as string[])],
    stops: [...new Set((a.informedEntity ?? []).map((e) => e.stopId).filter(Boolean) as string[])],
...
function mapSeverity(level?: string): AlertSeverity {
  if (level === "SEVERE") return "SEVERE" as AlertSeverity;
  if (level === "WARNING") return "WARNING" as AlertSeverity;
...
function mapEffect(effect?: string): AlertEffect {
  switch (effect) {
    ...
    case "ADDITIONAL_SERVICE": return "EXTRA_SERVICE" as AlertEffect;
    case "MODIFIED_SERVICE": return "MODIFIED_SERVICE" as AlertEffect;
    case "OTHER_EFFECT": return "OTHER_EFFECT" as AlertEffect;
    case "UNKNOWN_EFFECT": return "OTHER_EFFECT" as AlertEffect;
    case "STOP_MOVED": return "STOP_MOVED" as AlertEffect;
    case "NO_EFFECT": return "OTHER_EFFECT" as AlertEffect;
    case "ACCESSIBILITY_ISSUE": return "OTHER_EFFECT" as AlertEffect;
